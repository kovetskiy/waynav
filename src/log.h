...
#define log_err(...)  \
    log_msg(LOG_LEVEL_ERROR, __FILE__, __LINE__, __VA_ARGS__)
#define log_warn(...) \
    log_msg(LOG_LEVEL_WARN,  __FILE__, __LINE__, __VA_ARGS__)
#define log_info(...) \
    log_msg(LOG_LEVEL_INFO,  __FILE__, __LINE__, __VA_ARGS__)
#define log_debug(...)                                         \
    do {                                                       \
        if (log_threshold >= LOG_LEVEL_DEBUG)                   \
            log_msg(LOG_LEVEL_DEBUG, __FILE__, __LINE__,       \
                    __VA_ARGS__);                               \
    } while (0)
